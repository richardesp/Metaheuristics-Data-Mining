...
def basic_genetic(pattern_length: int, n_solutions: int, n_generations: int, c_prob: float, m_prob: float, k: int,
                  last_poblation: list) -> None:
    verbose = False  # Modo verbose para imprimir
    ...
    # Solamente se pasan por referencia los valores del array, por tanto debemos rellenarla manualmente
    for individual in poblation:
        last_poblation.append(individual)
...
def main():
    ...
    thread_1 = threading.Thread(target=basic_genetic,
                                args=(7, sub_patterns_n_solutions, 100, .7, .4, 3, first_genetic,))

    thread_2 = threading.Thread(target=basic_genetic,
                                args=(7, sub_patterns_n_solutions, 100, .7, .4, 3, second_genetic,))

    print("Ejecutando paralelamente los algoritmos genéticos")
    thread_1.start()
    thread_2.start()
    ...
    # Ahora concatenaremos todas las posibles soluciones, generando un total de sub_pattern * 2

    poblation = []

    # Con esto aplicamos el principio de optimalidad de Belman
    for index in range(len(first_genetic)):
        poblation.append(
            [first_genetic[index][0] + second_genetic[index][0],
             evaluate_pattern(first_genetic[index][0] + second_genetic[index][0], data)])

        poblation.append(
            [second_genetic[index][0] + first_genetic[index][0],
             evaluate_pattern(second_genetic[index][0] + first_genetic[index][0], data)])
